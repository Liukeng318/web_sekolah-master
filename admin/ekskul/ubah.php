					...
					<div class="card-body">
						<form method="POST" action="proses_ubah.php?id=<?= $jurusan['id'] ?>">
							<div class="form-group">
								...
								<input type="text" value="<?= $jurusan['nama_jurusan'] ?>" class="form-control"
									id="nama_jurusan" placeholder="nama jurusan" autocomplete="off" required="required"
									name="nama_jurusan">
							</div>
							<div class="form-group">
								...
								<select name="kaprodi" id="kaprodi" class="form-control">
									<?php while ($row = mysqli_fetch_assoc($query_guru)): ?>
										<option value="<?= $row['id'] ?>" <?= $jurusan['ka_prodi'] == $row['id'] ? 'selected' : '' ?>>
											<?= $row['nama'] ?>
										</option>
									<?php endwhile; ?>
								</select>
